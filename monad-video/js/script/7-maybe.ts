...
function ex1() {
    const res = trySomething1().flatMap(x => trySomething2().flatMap(y => Maybe.just(x + y)));
}

function imaginaryDoNotation(): Maybe<number>{
    const x = await trySomething1();
    const y = await trySomething2();
