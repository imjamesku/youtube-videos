...
function ex1() {
  const res = trySomething1().flatMap((x) =>
    trySomething2().flatMap((y) => Maybe.unit(x + y))
  );
}

async function imaginaryDoNotation(): Maybe<number> {
  const x = await trySomething1(); // Maybe<number>
  const y = await trySomething2(); // Maybe<number>
  return x + y;
}
